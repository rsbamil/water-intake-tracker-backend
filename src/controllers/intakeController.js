...
const getTodayIntake = async (req, res, next) => {
    try {
      ...
      const [result, user] = await Promise.all([
        ...
        User.findById(userId).select("dailyGoal"),
      ]);
      ...
      const totalIntake = result.length > 0 ? result[0].total : 0;
      const dailyGoal = user.dailyGoal || 2000;
  
      const progress = Math.min(
        Math.round((totalIntake / dailyGoal) * 100),
        100
      );
      ...
      return res.status(200).json({
        ...
        data: {
          ...
          remaining,
          progress,
          goalReached: totalIntake >= dailyGoal,
        },
      });
    } catch (error) {
      next(error);
